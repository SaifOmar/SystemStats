...
- Removed duplicate `f2fs` entry (and nonexistent `fat32`) from the recognized filesystem set
- `temp()` now reads hwmon files with context managers so descriptors are closed promptly
